...
class TestMapper:
    def test_from_ansible_not_admin(self):
        user_dict = dict(name="name", password="password", email="email", is_admin=False)
        user_obj = User.from_ansible(user_dict)
        assert user_obj.name == "name" and user_obj.password == "password" and user_obj.email == "email" and user_obj.is_admin is False

    def test_from_ansible_admin(self):
        user_dict = dict(name="name", password="password", email="email", is_admin=True)
        user_obj = User.from_ansible(user_dict)
        assert user_obj.name == "name" and user_obj.password == "password" and user_obj.email == "email" and user_obj.is_admin is True

    def test_from_maas_superuser(self):
        user_dict = dict(username="name", is_local=False, is_superuser=True, email="email")
        user_obj = User.from_maas(user_dict)
        assert user_obj.name == "name" and user_obj.password == None and user_obj.email == "email" and user_obj.is_admin is True

    def test_from_maas_not_superuser(self):
        user_dict = dict(username="name", is_local=False, is_superuser=False, email="email")
        user_obj = User.from_maas(user_dict)
        assert user_obj.name == "name" and user_obj.password == None and user_obj.email == "email" and user_obj.is_admin is False

    def test_to_ansible_admin(self):
        user_dict = dict(username="name", is_local=False, is_superuser=True, email="email")
        user_obj = User.from_maas(user_dict)
        user_to_ansible = user_obj.to_ansible()
        assert user_to_ansible == dict(name="name", is_local=False, is_admin=True, email="email")

    def test_to_ansible_not_admin(self):
        user_dict = dict(username="name", is_local=False, is_superuser=False, email="email")
        user_obj = User.from_maas(user_dict)
        user_to_ansible = user_obj.to_ansible()
        assert user_to_ansible == dict(name="name", is_local=False, is_admin=False, email="email")

    def test_to_maas_not_admin(self):
        user_dict = dict(name="name", password="password", is_admin=False, email="email")
        user_obj = User.from_ansible(user_dict)
        user_to_maas = user_obj.to_maas()
        assert user_to_maas == dict(username="name", email="email", is_superuser=False, password="password")

    def test_to_maas_admin(self):
        user_dict = dict(name="name", password="password", is_admin=True, email="email")
        user_obj = User.from_ansible(user_dict)
        user_to_maas = user_obj.to_maas()
        assert user_to_maas == dict(username="name", email="email", is_superuser=True, password="password")
...
class TestSendRequestAndPayload:
    def test_payload_for_create_when_is_superuser(self):
        ...
        assert results == dict(username="name", password="password", email="email", is_superuser=1)
    
    def test_payload_for_create_when_is_not_superuser(self):
        user_dict = dict(name="name", password="password", email="email", is_admin=False)
        user_obj = User.from_ansible(user_dict)
        results = user_obj.payload_for_create()
        assert results == dict(username="name", password="password", email="email", is_superuser=0)
